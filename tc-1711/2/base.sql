...
INNER JOIN brokers parent ON brokers.parent_id = parent.id
WHERE email_ver = 1 AND a.id NOT IN (SELECT srcims_id FROM srcims_deposit WHERE srcims_id IS NOT NULL GROUP BY srcims_id)
